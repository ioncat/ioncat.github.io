...
Career Agent — карьерный советник для PdM / PO.

PM-рынок конкурентный и становится ещё конкурентнее. Типичный кандидат тратит 30–90 минут на доработку CV под вакансию — до того как узнаёт, есть ли у него вообще шанс. При активном поиске — 30, 70 вакансий — это недели усилий без гарантии качества.

Две проблемы усиливают друг друга. Первая: кандидаты читают JD (job description — описание вакансии) эмоционально — первое впечатление якорит восприятие, реальные барьеры упускаются. Реальный пример: кандидат оценил себя на 10/10 — агент вернул 4/10. Дельта = 6 баллов из-за одного скрытого требования. Вторая: даже когда решение подавать правильное, написать по-настоящему адаптированный CV вручную — это время, которое большинство кандидатов не тратит. Они переиспользуют один документ и удивляются, почему response rate низкий.

**Основной ICP:** PdM (Product Manager) · PO (Product Owner) в активном поиске.  
**Расширенный ICP:** PM (Project Manager) · BA (Business Analyst) · другие нетехнические роли (через generic skill type).

---

## Видение продукта

Career Agent строит портрет кандидата — через структурированный онбординг, данные LinkedIn/CV и доказательства, всплывающие в ходе пайплайна. Эти знания накапливаются с каждой сессией.

Когда фит есть: целевой CV из реального опыта, сверенный с требованиями вакансии, доставленный в PDF. Никаких выдуманных утверждений. Пользователь одобряет — или просит правки. Всё остальное автоматически.

Кандидаты тратят время на решения, а не на написание.

**Проблемы, которые мы решаем:**

- Кандидат не знает реальных шансов до отправки CV
- 30–90 минут на вакансию × 30–70 вакансий = недели усилий без гарантии качества
- JD читается эмоционально — первое впечатление якорит, реальные барьеры упускаются
- Самооценка соответствия систематически завышена (реальный пример: 10/10 → 4/10)
- Один CV переиспользуется — response rate низкий, причина непонятна
- CV не отвечает на реальную боль работодателя, скрытую в JD
- Нет честного go/no-go до генерации CV
- Generic инструменты не понимают PM-архетипы (Founder Proxy vs Executor, Discovery vs Delivery)
- Пробелы не закрыты до начала написания CV
- CV-генераторы выдумывают — нет верифицированного профиля, нет накопленных доказательств

---

## Решение

**Career Agent** — AI-пайплайн на базе Claude API (Anthropic), созданный для самой сложной части поиска работы: оценить соответствие до того, как вкладываешь усилия в отклик. Не генератор CV — пайплайн, который отвечает на два вопроса по порядку.
...
| **Claude API + Extended Thinking** | Основной анализ использует Extended Thinking для выявления неочевидных барьеров; prompt caching снижает стоимость на вакансию |
...
Cost of Service ~$0.32 на вакансию (полный pipeline: fetch → analyze → CV → cover) на Claude Sonnet 4.6 с prompt caching.

| Модель | Revenue | Cost of Service | Gross Margin |
|---|---|---|---|
...
Deterministic/Cognitive split — выделить детерминированную оркестрацию (файловые операции, dedup, рендеринг PDF) в Python FSM; вызывать LLM только на 3 когнитивных точках вместо ~43 смешанных шагов. Цель: снижение latency, снижение Cost of Service, предсказуемый output.
