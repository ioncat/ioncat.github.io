...
**Стоит ли подавать?** Агент читает JD глубже, чем кандидат под влиянием первого эмоционального впечатления. Извлекает реальную боль работодателя, скрытые требования, архетип роли (Founder Proxy vs Executor). Оценивает вакансию по 8 измерениям (**VScore** — привлекательность вакансии) и запускает матрицу Fit × VScore. Вердикт: `apply` · `take a chance` · `decline`. `decline` останавливает пайплайн — усилия не тратятся.
...
3. **Fit Scoring (Phase 2)** — матрица Fit × VScore → вердикт: `apply` · `take a chance` · `decline`. Барьеры + Adaptation Plan. `decline` → пайплайн останавливается, CV не тратится.
